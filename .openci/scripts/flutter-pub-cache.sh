...
local_cache_root() {
  if [ -z "${OPENCI_FLUTTER_PUB_LOCAL_CACHE_DIR:-}" ]; then
    return 1
  fi
  if [ ! -d "$OPENCI_FLUTTER_PUB_LOCAL_CACHE_DIR" ]; then
    return 1
  fi
  printf '%s' "$OPENCI_FLUTTER_PUB_LOCAL_CACHE_DIR"
}
...
prune_local_cache() {
  local root
  local max_bytes="${OPENCI_FLUTTER_PUB_LOCAL_CACHE_MAX_BYTES:-10737418240}"
...
restore_local_cache() {
  ...
  if [ ! -f "$local_archive_path" ]; then
    echo "Flutter pub local cache miss"
    return 1
  fi

  echo "Restoring Flutter pub cache from local archive: $local_archive_path"
  du -sh "$local_archive_path"
  if ! extract_archive "$local_archive_path"; then
    echo "Flutter pub local cache restore failed; falling back to remote cache" >&2
...
save_local_cache() {
  ...
  local_archive_path="$(local_cache_archive_path "$object_name")" || return 0
  if [ -f "$local_archive_path" ]; then
    echo "Flutter pub local cache already exists; skipping local save: $local_archive_path"
    update_local_cache_metadata "$object_name" "$local_archive_path" "save-skip"
    return 0
  fi

  if [ ! -d "$cache_dir" ]; then
    echo "Flutter pub cache not found; nothing to save locally: $cache_dir"
    return 0
  fi

  archive_path="$tmp_dir/flutter-pub-cache-local.$(compression_extension)"
  echo "Creating Flutter pub local cache archive:"
  du -sh "$cache_dir"
...
restore_cache() {
  ...
  if ! store_local_archive "$object_name" "$archive_path" "remote-seed"; then
    echo "Warning: failed to seed local Flutter pub cache" >&2
  fi
...
save_cache() {
  ...
  if [ "$remote_exists" = "true" ] && [ "$local_exists" = "true" ]; then
    echo "Flutter pub cache already exists locally and remotely; skipping save"
    update_local_cache_metadata "$object_name" "$local_archive_path" "save-skip"
    return 0
  fi
...
if [ -z "${FIREBASE_SERVICE_ACCOUNT:-}" ]; then
  ...
  echo "FIREBASE_SERVICE_ACCOUNT is not set; skipping remote Flutter pub cache ${action}"
  exit 0
fi
