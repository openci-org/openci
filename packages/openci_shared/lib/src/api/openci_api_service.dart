...
abstract class OpenCiApiService extends ChopperService {
  ...
  @GET(path: '/teams', timeout: Duration(seconds: 10))
  Future<Response<List<Team>>> getTeams();

  @POST(path: '/teams', timeout: Duration(seconds: 10))
  Future<Response<Map<String, dynamic>>> createTeam(
    @Body() Map<String, dynamic> body,
  );

  @PATCH(path: '/teams/{id}', timeout: Duration(seconds: 10))
  Future<Response<void>> updateTeam(
    @Path('id') String id,
    @Body() Map<String, dynamic> body,
  );

  @DELETE(path: '/teams/{id}', timeout: Duration(seconds: 10))
  Future<Response<void>> deleteTeam(@Path('id') String id);

  @POST(path: '/teams/{id}/members', timeout: Duration(seconds: 10))
  Future<Response<void>> inviteMember(
    @Path('id') String id,
    @Body() Map<String, dynamic> body,
  );

  @GET(path: '/devices', timeout: Duration(seconds: 10))
  Future<Response<List<UserDevice>>> getDevices();

  @GET(path: '/workers', timeout: Duration(seconds: 10))
  Future<Response<Map<String, dynamic>>> getWorkers();

  @POST(path: '/builds/claim', timeout: Duration(seconds: 10))
  Future<Response<Map<String, dynamic>>> claimNextJob(
    @Body() Map<String, dynamic> body,
  );

  @POST(path: '/builds/{id}/runs', timeout: Duration(seconds: 10))
  Future<Response<void>> createRun(
    @Path('id') String buildJobId,
    @Body() Map<String, dynamic> body,
  );

  @PATCH(path: '/builds/{id}', timeout: Duration(seconds: 10))
  ...
  @GET(path: '/builds/{id}', timeout: Duration(seconds: 10))
  Future<Response<Map<String, dynamic>>> getBuildJob(
    @Path('id') String id,
  );

  @POST(path: '/builds/{id}/check-run', timeout: Duration(seconds: 10))
  Future<Response<void>> updateCheckRun(
    @Path('id') String buildJobId,
    @Body() Map<String, dynamic> body,
  );

  @POST(path: '/builds/{id}/status-change', timeout: Duration(seconds: 10))
  Future<Response<void>> handleBuildJobStatusChange(
    @Path('id') String buildJobId,
    @Body() Map<String, dynamic> body,
  );

  @GET(path: '/builds/{id}/token', timeout: Duration(seconds: 10))
  Future<Response<Map<String, dynamic>>> resolveInstallationToken(
    @Path('id') String buildJobId,
  );

  @POST(path: '/teams/{teamId}/secrets', timeout: Duration(seconds: 10))
  Future<Response<void>> saveSecret(
    @Path('teamId') String teamId,
    @Body() Map<String, dynamic> body,
  );

  @GET(path: '/teams/{teamId}/secrets', timeout: Duration(seconds: 10))
  Future<Response<Map<String, dynamic>>> getSecrets(
    @Path('teamId') String teamId,
  );

  @GET(path: '/teams/{teamId}/secrets/{name}', timeout: Duration(seconds: 10))
  Future<Response<Map<String, dynamic>>> getSecretValue(
    @Path('teamId') String teamId,
    @Path('name') String name,
  );

  @POST(path: '/workers/heartbeat', timeout: Duration(seconds: 10))
