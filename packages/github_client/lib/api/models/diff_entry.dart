...
abstract class DiffEntry with _$DiffEntry {
  const factory DiffEntry({
    ...
    @JsonKey(name: 'Status') required DiffEntryStatus status,
    required int additions,
    required int deletions,
    required int changes,
    @JsonKey(name: 'blob_url') required String blobUrl,
    @JsonKey(name: 'raw_url') required String rawUrl,
    @JsonKey(name: 'contents_url') required String contentsUrl,
    String? patch,
    @JsonKey(name: 'previous_filename') String? previousFilename,
  }) = _DiffEntry;

  factory DiffEntry.fromJson(Map<String, Object?> json) =>
