...
void main() {
  group('ApiClient Tests', () {
    ...
    test('getSecrets queries team secrets endpoint and parses response', () async {
      ...
      final done = server.listen((HttpRequest request) {
        ...
        expect(request.headers.value('Authorization'), equals('Bearer mock-id-token'));
        expect(request.headers.value('Content-Type'), equals('application/json'));

        final responsePayload = [
          {'id': '1', 'name': 'SSH_KEY', 'createdAt': '2026-06-16T12:00:00Z', 'updatedAt': '2026-06-16T12:00:00Z'},
          {'id': '2', 'name': 'API_KEY', 'createdAt': '2026-06-16T12:00:00Z', 'updatedAt': '2026-06-16T12:00:00Z'}
        ];
    ...
    test('getSecretValue queries team secret value endpoint and returns the value', () async {
      const secretName = 'API_KEY';
      const secretValue = 'my-decrypted-secret-token';

      final done = server.listen((HttpRequest request) {
        expect(request.method, equals('GET'));
        expect(request.uri.path, equals('/teams/$teamId/secrets/$secretName/value'));
        expect(request.headers.value('Authorization'), equals('Bearer mock-id-token'));
